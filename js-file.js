...
let displayElement = document.getElementById('display-txt');
let display = '';
...
numberButtons.forEach((button) => {
    button.addEventListener('pointerdown', () => {
        addNumberToDisplay(button.innerText);
        button.style.backgroundColor = '#484b54';
    });
});
...
function resetColor() {
    numberButtons.forEach((button) => {
        button.style.backgroundColor = '#9299a9';
    })
}
...
function addNumberToDisplay(number) {
    if(addsToFirstNumber) {
        if(firstNumberArray.includes('.') && number === '.') {
            return; 
        }
        firstNumberArray.push(number);
        setDisplay(firstNumberArray.join(''));
    } else {
        if(secondNumberArray.includes('.') && number === '.') {
            return }
        secondNumberArray.push(number);
        setDisplay(secondNumberArray.join(''));
    }
}
